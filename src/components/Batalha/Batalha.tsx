...
export default function Batalha() {
  ...
  const [itensJ1, setItensJ1] = useState([
    ...
    { nome: "Welcome", cura: 50, recuperaPP: 3, uso: 1 },
  ]);

  const [itensJ2, setItensJ2] = useState([
    { nome: "Elidio", cura: 5, recuperaPP: 5, uso: 3 },
    { nome: "Semente", cura: 10, recuperaPP: 5, uso: 2 },
    { nome: "Welcome", cura: 50, recuperaPP: 5, uso: 1 },
  ]);

  const [ataquesJ1, setAtaqueJ1] = useState([
    { nome: "Garra de Dragão", dano: 5, precisao: 75, pp: 5, ppMax: 5 },
    { nome: "Rajada de Fogo", dano: 5, precisao: 80, pp: 5, ppMax: 5 },
    { nome: "Golpe de Ar", dano: 5, precisao: 70, pp: 5, ppMax: 5 },
  ]);

  const [ataquesJ2, setAtaqueJ2] = useState([
    { nome: "Thunderstorm", dano: 5, precisao: 75, pp: 5, ppMax: 5 },
    { nome: "Electro Ball", dano: 5, precisao: 90, pp: 5, ppMax: 5 },
    { nome: "Ataque Rápido", dano: 5, precisao: 80, pp: 5, ppMax: 5 },
  ]);
  ...
  function usarItem(atacante: "J1" | "J2", index: number) {
    ...
    if (atacante === "J1" && turno % 2 !== 0 && itemJ1[index].uso > 0) {
      ...
      for (let i = 0; i < ataqueJ1.length; i++){
        ataqueJ1[i].pp = Math.min(ataqueJ1[i].pp + itemJ1[index].recuperaPP, ataqueJ1[i].ppMax)
      }
      itemJ1[index].uso -= 1;
      setItensJ1(itemJ1);
      setAtaqueJ1(ataqueJ1);
      ...
      for (let i = 0; i < ataqueJ2.length; i++){
        ataqueJ2[i].pp = Math.min(ataqueJ2[i].pp + itemJ2[index].recuperaPP, ataqueJ2[i].ppMax)
      }
      itemJ2[index].uso -= 1;
      setItensJ2(itemJ2);
      setAtaqueJ2(ataqueJ2);
